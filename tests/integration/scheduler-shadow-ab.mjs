#!/usr/bin/env node
/**
 * scheduler-shadow-ab.mjs — Phase 17.6 A/B compatibility harness.
 *
 * Boots the OnCall backend twice from the SAME server.js:
 *   A) LEGACY   (no platform flags)
 *   B) ENTERPRISE + SCHEDULER SHADOW  (PLATFORM_ENABLED=1, PLATFORM_HOST=1,
 *                                      PLATFORM_SCHEDULER=1, SHADOW_SCHEDULER=1)
 * and asserts the HTTP responses are byte-identical. The Scheduler shadow runs out-of-band at
 * boot and NEVER calls start()/tick(), so scheduling/timing/ownership are unchanged and no
 * observable behavior differs. The legacy scheduler remains the only owner of timing.
 *
 * Requires the sqlite3 native binding (run on the app's normal OS / CI). Prints
...
const TMP = mkdtempSync(join(tmpdir(), 'oncall-sched-ab-'));
const JWT_SECRET = process.env.JWT_SECRET || 'sched-shadow-ab-secret-0123456789abcdef';

const A = { name: 'legacy', port: 4841, db: join(TMP, 'a.db'), env: {} };
const B = {
  name: 'enterprise+scheduler-shadow',
  port: 4842,
  db: join(TMP, 'b.db'),
  env: { PLATFORM_ENABLED: '1', PLATFORM_HOST: '1', PLATFORM_SCHEDULER: '1', SHADOW_SCHEDULER: '1' },
};
...
console.log(failures === 0 ? '\nResult: IDENTICAL — scheduler shadow changes zero observable behavior; never owns a timer or executes' : `\nResult: DRIFT — ${failures} probe(s) differ`);
